...
function curl_exec(CurlHandle $handle)/*: string|bool*/
{
	...
	if (null !== $handle->getOpt(CURLOPT_POSTFIELDS))
	{
		...
		if (1 == $ControllerType)
		{
			$controller1 = @json_decode($handle->getOpt(CURLOPT_POSTFIELDS), true);
			if (defined('DEBUG') && DEBUG)
			{
	...
	// login phase --> check credentials
	if ("/api/login" == $url &&
	(((!isset($controller0[0]) || $UserName != $controller0[0]) && (!isset($controller1['username']) || $UserName != $controller1['username']))
	|| ((!isset($controller0[1]) || $Password != $controller0[1]) && (!isset($controller1['password']) || $Password != $controller1['password']))))
	{
		$httpCode = '401';
		if (null !== $handle->getOpt(CURLOPT_HEADER) && true == $handle->getOpt(CURLOPT_HEADER))
		{
