...
{
	private $CURLOPT = array(CURLOPT_WRITEHEADER);
	private $CURLINFO = array(CURLINFO_EFFECTIVE_METHOD);
}
...
/*
curl_setUnifiController
Close a cURL session
return: void
 */
function curl_setReturn(string $newReturn): void
{
	global $returnValue;
...
function curl_exec(CurlHandle $handle)/*: string|bool*/
{
	...
	if (!isset($handle->$CURLOPT[CURLOPT_URL]))
	{
		return false;
	}

	$url = $handle->$CURLOPT[CURLOPT_URL];
	...
	if (
		...
		$httpCode = '404';
		if (isset($handle->$CURLOPT[CURLOPT_HEADER]) && true == $handle->$CURLOPT[CURLOPT_HEADER])
		{
			...
			$handle->$CURLINFO[CURLINFO_HEADER_SIZE] = strpos($returnValue, "httponly");
			$handle->$CURLINFO[CURLINFO_HTTP_CODE] = $httpCode;
			return $returnValue;
		}
		else
		{
	...
	if (isset($handle->$CURLOPT[CURLOPT_POSTFIELDS]))
	{
		echo "\ncurl_exec CURLOPT_POSTFIELDS: ".$handle->$CURLOPT[CURLOPT_POSTFIELDS];
		$controller0 = @explode("&", $handle->$CURLOPT[CURLOPT_POSTFIELDS]);
		$controller0 = str_ireplace(array("username=", "password="), array("", ""), $controller0);
		echo "\ncurl_exec Controller0:";
		print_r($controller0);
		$controller1 = @json_decode($handle->$CURLOPT[CURLOPT_POSTFIELDS]);
	...
	if ("/api/login" != $url)
	{
		if (isset($handle->$CURLOPT[CURLOPT_HTTPHEADER]))
		{
			foreach ($handle->$CURLOPT[CURLOPT_HTTPHEADER] as $httpHeader)
			{
...
{
	if (isset($handle->$CURLINFO[$option]))
	{
		return (int)$handle->$CURLINFO[$option];
	}
	else
	{
		return false;
